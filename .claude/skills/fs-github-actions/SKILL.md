...
| `project-id`      | no       | from setup          | Platform project ID (signed 64-bit integer, not a UUID). Used with `version` when no version ID is known; skips the name lookup, outranks `project-name` |
...
- **An explicit project input beats an inherited project ID.** The two can name different projects, so when you pass `project-name` the action sends `--name` alone rather than letting fs-cli choose between them. `project-id` outranks `project-name` when both are given, because an ID identifies exactly one project where a name can match several. It is a platform project ID — a signed 64-bit integer such as `-4065045466680884751`, the same value `scan` and `upload` export as `FINITE_STATE_PROJECT_ID` — not a UUID.
