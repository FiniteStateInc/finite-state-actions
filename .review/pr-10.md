...
Three, all deliberate. Two things that looked like changes are not: the `::error title=…::` annotations and fs-cli's exit code are both preserved (see below).
...
- **An unsupported runner now fails even with a working fs-cli on `PATH`.** `ensureFsCli` resolves the runner's OS and architecture before it looks at `PATH`, so anything outside linux/darwin/windows on amd64/arm64 fails where the shell version ran whatever `command -v fs-cli` found. `scan` and `upload` have always behaved this way; `wait` now matches them. Being the first Finite State step in a job also means needing egress to the platform's CLI endpoint. Both are in SKILL.md's gotchas, with troubleshooting rows.
- **`timeout` is parsed by shared code, and a leading zero is base 10.** `timeoutSecondsToMinutes` in `packages/core/src/timeout.ts`, extracted from `upload`, which had the same 20 lines inline. Same rejections (`600s`, `10 minutes`, `-5`, `0`) and same rounding warning in both actions. `0600` now means 600 seconds, where bash read it as octal 384 — a workflow that happened to rely on the octal reading waits 10 minutes instead of 6.4. It returns the rounding message rather than writing it, so each action titles its own annotation and core needs no Actions logging channel.

### What is preserved, despite the rewrite

- **fs-cli's exit code.** `wait.sh` ended with `exit "$QUERY_EXIT"`. `core.setFailed` would force 1 and lose the difference between a failed scan and a poll timeout, so the failure path sets `process.exitCode` to fs-cli's own code instead.
- **The titled annotations.** `WaitFailure` in `actions/wait/src/main.ts` carries the title each failure had in the shell version — `No API token`, `No version ID`, `Bad timeout`, `fs-cli not found`, `Scan did not finish`, and `Timeout rounded` on the rounding warning — and the catch writes it through `core.error`, which can carry one where `setFailed` cannot. A log filter keyed on `title=Scan did not finish` still matches. The message bodies did change: the missing-version message is now `No version to wait on.`, and SKILL.md is updated for it.
- **Every context value is trimmed.** `core.getInput` trims its result but an environment variable does not, so `readSetupContext` now trims and treats a whitespace-only value as absent — a token assembled from a `vars.` expression no longer reaches the API with a trailing newline, and a blank `FINITE_STATE_VERSION_ID` no longer counts as a version to query. This applies to every action, not just `wait`.
...
- it runs exactly the binary path `ensureFsCli` returns, checked for four paths including `D:\a\_temp\fs-cli\fs-cli.exe` and `C:\Program Files\fs-cli\fs-cli.exe`;
...
`actions/wait/__tests__/spawn.test.ts` is the one test in the repo that spawns a real process. It creates a directory whose name contains a space, puts an executable in it, and runs it through `@actions/exec` — asserting the exit code comes back through the quoted path, and that the unquoted path fails. Asserting the quoting against `@actions/exec`'s own parser would only prove the parser agrees with itself; this proves the path survives to `spawn`, on whichever OS the leg is running.

`packages/core/__tests__/install-cli.test.ts` adds three `ensureFsCli` cases with `process.platform` stubbed to `win32`, so the `['.exe', '']` branch of `findOnPath` — on `wait`'s critical path now — is exercised on every leg rather than only the Windows one. `path.join` and `path.delimiter` still follow the host, so drive letters and `;`-separated `PATH` entries remain the Windows leg's job. `.cmd` is gone from that list: a batch file cannot pass the executable-header check, and it is the one resolution where `@actions/exec` goes through the Windows command interpreter — dropping it makes the "never a shell" property true rather than nearly true. `packages/core/__tests__/timeout.test.ts` and `exec-path.test.ts` are new: both helpers are exported from `@finite-state/core` and had no test in the package that owns them.
...
- **`actions/upload-scan/action.yml` still declared `shell: bash`** for its deprecation notice, which is the failure this PR exists to remove. The notice now rides on `deprecationMessage`, which GitHub logs itself on every runner OS, and the composite is one `uses:` step with no `run:` step and no shell. It is on both `type` and `file` rather than one input: GitHub does not enforce `required`, and it logs a `deprecationMessage` only for an input the caller actually passed, so those two — the inputs an upload cannot work without — are where it will be seen.
- **The `build` job's dist check could not see a missing bundle, in two ways.** It used `git diff`, which reports nothing for a file git does not track, and the root `.gitignore` lists `dist/` and `*.js`, so a new action's bundle starts out untracked. It also globbed `actions/*/dist/`, which contributes no path at all for an action whose `dist/` was never created. There are now two steps: `git status --porcelain --ignored=matching actions/*/dist/` for staleness, and a loop asserting a tracked `dist/index.js` for every `action.yml` that declares a Node runtime.
...
+  })
+})
diff --git a/actions/wait/__tests__/spawn.test.ts b/actions/wait/__tests__/spawn.test.ts
new file mode 100644
index 0000000..6b96b1b
--- /dev/null
+++ b/actions/wait/__tests__/spawn.test.ts
@@ -0,0 +1,68 @@
