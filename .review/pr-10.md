...
+      # git status, not git diff: the root .gitignore lists dist/ and *.js, so a
+      # new action's bundle starts out untracked, and git diff reports nothing
+      # for a file git does not track yet. That would ship an action with no
+      # dist/index.js and a green build.
...
+
 ...
 ```bash
@@ -49,13 +51,14 @@ cd actions/setup && pnpm build
...
@@ -79,8 +102,9 @@ describe('scan action', () => {
...
+      expect.any(String),
       [
         'scan',
         '--endpoint',
@@ -154,13 +178,14 @@ describe('scan action', () => {
...
@@ -269,8 +294,10 @@ describe('scan action', () => {
...
@@ -14,6 +14,11 @@ inputs:
   type:
     ...
     required: true
+    # Carries the whole action's deprecation notice, because `type` is required
+    # and so always set. GitHub logs it itself, on every runner OS — where the
...
+import { argStringToArray } from '@actions/exec/lib/toolrunner'
 ...
 // ── Helpers ────────────────────────────────────────────────────────────────────
@@ -133,7 +148,9 @@ describe('upload action', () => {
     await run()
...
+  const exitCode = await exec.exec(quoteExecPath(binary), args, {
     ...
     listeners: {
@@ -289,30 +298,10 @@ export async function run(): Promise<void> {
...
@@ -62,6 +63,23 @@ function pe(machine = 0x8664): Uint8Array {
...
@@ -102,7 +120,7 @@ function stubPlatform(platform: string, arch: string) {
...
@@ -120,11 +138,11 @@ describe('installFsCli', () => {
...
@@ -250,8 +268,8 @@ describe('installFsCli', () => {
...
@@ -264,14 +282,14 @@ describe('ensureFsCli', () => {
...
@@ -279,7 +297,7 @@ describe('ensureFsCli', () => {
...
+      if (candidate !== ON_PATH) throw new Error('ENOENT')
     ...
     onDisk(pe())
     const client = makeClient()
@@ -290,13 +308,13 @@ describe('ensureFsCli', () => {
...
+      if (candidate !== ON_PATH) throw new Error('ENOENT')
     ...
     const client = makeClient()
@@ -304,7 +322,7 @@ describe('ensureFsCli', () => {
     const binary = await ensureFsCli(client)
...
@@ -315,6 +333,57 @@ describe('ensureFsCli', () => {
     const binary = await ensureFsCli(client)
